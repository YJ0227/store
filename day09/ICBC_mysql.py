...
import random
from ICBC_DBUtils import *

bank_name = "中国工商银行昌平支行"  # 银行名称写死的
...
def bank_addUser(username):
    ...
    param = []
    select_username = select(sql_username, param)
...
def addUser():
    ...
    while True:
        ...
        param = []
        select_account = select(sql_account, param)
        if account not in select_account:
            break

    status = bank_addUser(username)
    if status == 3:
...
def saveMoney():
    ...
    for i in select_account:
        if account in i:
            ...
            param = [account]
            select_info = select(sql_info, param)
            if password == select_info[0][2]:
                ...
                update(sql_save, param_mon_acc)
                print("存款成功！")
                sql_new_info = "select * from user where account = %s"
                param_mon_acc_new = [account]
                new_info = select(sql_new_info,param_mon_acc_new)
...
def getMoney():
    ...
    for i in select_account:
        if account in i:
            ...
            select_info = select(sql_info, param_input_account)
            if password == select_info[0][2]:
                ...
                if get_money <= select_info[0][7]:
                    ...
                    update(sql_get, param_input_money)
                    print("取款成功！")
                    print("成功取款%s元" % get_money)
                    sql_new_info = "select * from user where account = %s"
                    param_new_money = [account]
                    new_info = select(sql_new_info, param_new_money)
                    print("您的余额为：%s元" % new_info[0][7])
                else:
                    Return(3)
            else:
                    ...
                    for j in select_account:
                        if in_account in j:
                            ...
                            if out_money <= select_info[0][7]:
                                ...
                                update(sql_transfer_out, param_out)
                                update(sql_transfer_in, param_in)
                                print("转账成功！")
                                sql_new_info = "select * from user where account = %s"
                                param_out = [out_account]
                                new_info = select(sql_new_info,param_out)
                                print("您的余额为：%s元" % new_info[0][7])
                            else:
                                Return(3)
                            break
        ...
        if account in i:
            ...
            if password == select_info[0][2]:
                info = '''
                        ...
                        --------------------------------
                        '''
                print(info % (select_info[0][0], select_info[0][1], select_info[0][2], select_info[0][3], select_info[0][4], select_info[0][5], select_info[0][6], select_info[0][7], select_info[0][9]))
